"""Penalized regression (L1/L2/ElasticNet) for SyMANTIC.

Replaces the combinatorial L0 search with a regularization path sweep,
producing a Pareto front of (complexity, RMSE) solutions without
enumerating all C(k, n_terms) feature subsets.
"""
...
class PenalizedRegressor:
    """Penalized regression using L1 (Lasso), L2 (Ridge), or ElasticNet.

    Instead of exhaustively enumerating feature combinations (L0), this
    regressor sweeps a regularization path and lets the penalty drive
    sparsity.  For L1 and ElasticNet the number of non-zero coefficients
    is controlled by alpha; for L2 a relative threshold zeros out small
    coefficients.
    ...
    dimension : int or None
        Maximum number of terms.  Used to size the SIS screening pool
        (``sis_features * dimension``).
    sis_features : int
    ...
    quantiles : list or None
        Ignored (kept for interface compatibility with L0).
    regularization : str
        'l1', 'l2', or 'elastic_net'.
    reg_alpha : float or None
        Regularization strength.  ``None`` means auto-select via path.
    l1_ratio : float
        L1/L2 mixing for elastic_net (1.0 = pure L1).
    reg_threshold : float
        For L2: zero out coefficients whose absolute value is below
        ``reg_threshold * max(|coef|)``.
    n_alphas : int
        Number of alpha values in the regularization path.
    """

    def __init__(self, x, y, names, complexity, dimension=None,
                 sis_features=10, device='cpu', metrics=(0.06, 0.995),
                 disp=False, quantiles=None, *,
        ...
        self.y = y.to(device)
        self.names = names
        self.complexity = complexity.to(device)
        ...
        # Standardize features and center target (same as L0)
    ...
    def _solve_path(self, X, y):
        ...
        coef_path has shape (n_features, n_alphas).
        """
        ...
        # L2 (Ridge) — sweep alphas manually, threshold small coefficients
    ...
    def _path_to_pareto(self, coef_path, screened_idx, X_std, y_cen):
        ...
        For each alpha, compute RMSE, R2, complexity, denormalized
        coefficients, intercept, and feature names.
        Deduplicates solutions that select the same feature set (keeps
        the one with lowest RMSE).
        """
        ...
        seen_patterns = {}  # frozenset(nz_indices) -> index into results
        ...
        for j in range(n_alphas):
            ...
            if len(nz) == 0:
                continue  # skip all-zero (intercept-only added separately)

            # Limit to max terms
            if len(nz) > self.dimension:
                # Keep the largest |coef| features
            ...
            # Deduplicate: keep lowest RMSE for each sparsity pattern
            ...
            # Denormalize coefficients to original scale
            nz_std = x_std_s[nz]
            denorm_coefs = coefs[nz] / nz_std
            intercept = float(self.y_mean) - float(
                np.dot(x_mean_s[nz] / nz_std, coefs[nz])
    ...
    # ------------------------------------------------------------------
    # Format equation string (matches L0 output style)
    # ------------------------------------------------------------------
    @staticmethod
    ...
    def regressor_fit(self):
        ...
        # 5. Add intercept-only baseline (zero complexity)
        ...
        if len(rmses) < 2:
            # Only baseline — no regularization produced non-zero coefficients
        ...
        # 7. Build coefficient tensor (pad with NaN for ragged dimensions)
        max_terms = max(
        ...
        # 8. Select best model (utopia = closest to min RMSE + min complexity)
        # Find the non-baseline Pareto solution with lowest RMSE
        non_baseline = pareto_complexity > 0
        if non_baseline.any():
            best_idx = torch.argmin(pareto_rmse[non_baseline])
            # Map back to pareto arrays
        ...
        if self.disp:
            ...
            print(f'Penalized regression ({self.regularization}) completed '
                  f'in {elapsed:.2f}s — {len(rmses)} solutions, '
